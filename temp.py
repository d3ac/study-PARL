...
class PG(object):
    ...
    def choose_action(self, observation):
        ...
        network_output = self.network.forward(observation).cpu().detach().numpy()
        action = np.random.choice(len(network_output), 1, p=network_output)[0]
        return action
    ...
    def learn(self):
        obs = torch.Tensor(self.ep_obs).to(device)
        action = torch.LongTensor(self.ep_as).to(device)
        ...
        log_prob = Categorical(self.network(obs)).log_prob(action)
       
        # Step 3: 反向传播
        loss = torch.mean( - log_prob * discounted_ep_rs)
        ...
        # reward = np.zeros_like(self.ep_rs)
        # running_add = 0
        # for t in reversed(range(0, len(self.ep_rs))):
        #     running_add = running_add * GAMMA + self.ep_rs[t]
